...
def exportar_resultados(resultados, df_integrado):
    resultados['por_programa'].to_excel(BASE / 'resumen_por_programa.xlsx')
    resultados['por_asignatura'].to_excel(BASE / 'resumen_por_asignatura.xlsx')
    resultados['asignaturas_mayor_variabilidad'].to_excel(BASE / 'asignaturas_alta_variabilidad.xlsx')
    resultados['por_estudiante'].to_csv(BASE / 'promedio_por_estudiante.csv', index=False)
    df_integrado.to_csv(BASE / 'datos_integrados.csv', index=False)

    with open(BASE / 'porcentaje_en_riesgo.txt', 'w') as f:
        f.write(str(resultados['porcentaje_en_riesgo']))

    print("Resultados exportados en:", BASE)
