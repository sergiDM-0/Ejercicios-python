...
def funcion_hash(dato, tamanio_tabla):
    ...
    Basada en la descripción de la Página 121 [cite: 2647-2651].
    """
    
    # 1. Obtener la clave numérica 'k' (usando SHA-1 como en tu código original)
    k = Hash_sha256(str(dato))
    
    # 2. Aplicar la fórmula de multiplicación de Knuth [cite: 2647-2651]
    
    # Constante A según Knuth [cite: 2650-2651]
    A = 0.6180339887498949 
    # m = 2**32 (sugerido por el libro [cite: 2649])
    m = 4294967296

    # Fórmula: h(k) = floor( m * ((k * A) % 1) ) % tamaño_tabla
    
    # (k * A) % 1  (Esto obtiene la parte fraccionaria)
    fraccion = (k * A) % 1
    
    # floor( m * fraccion )
    resultado_m = int(m * fraccion)

    # ... % tamanio_tabla
    posicion = resultado_m % tamanio_tabla
